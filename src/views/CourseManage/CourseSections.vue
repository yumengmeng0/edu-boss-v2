  ...
  <section class="course-sections">
    <div class="header">
      <el-page-header
        ...
        :content="addSectionForm.courseName"
      />
      <el-button
        type="primary"
        icon="el-icon-plus"
    ...
    >
      <div class="inner" slot-scope="{ data, node }">
        ...
        <span class="actions" v-if="data.sectionName">
          <el-button size="small" @click.stop="handleEditSection(data)"
            >编辑</el-button
          >
          <el-button
            size="small"
            ...
            >添加课时</el-button
          >
          <el-button size="small" @click.stop="handleShowToggleStatus(data)">{{
        ...
        <span class="actions" v-else>
          <el-button
            size="small"
            @click.stop="handleEditLesson(data, node.parent.data)"
            >编辑</el-button
          >
          <!-- TODO: 上传视频 -->
    ...
    <el-dialog
      class="toggle-dialog"
      title="提示"
      :visible.sync="showToggleStatusForm"
      width="30%"
    >
      <header class="toggle-header">
        ...
        <span
          >当前状态：{{
            toggleStatusForm.data && statusMapping[toggleStatusForm.data.status]
          }}</span
        >
      </header>
      <el-form
...
export default {
  ...
  methods: {
    ...
    handleToggleStatus() {
      ...
      if (this.toggleStatusForm.data.sectionName) {
        ...
        axios
          ...
          .catch(error => {
            this.showStatusForm = false;
            this.$message.error("修改课时状态失败! ! !");
          });
        // console.log(this.toggleStatusForm);
      }
    },
    ...
    handleShowAddLesson(section) {
      ...
      // this.addLessonForm.courseId = section.courseId;
      // 清空上次显示的内容
      this.addLessonForm = {
        courseName: this.addSectionForm.courseName,
