...
export default {
  ...
  data() {
    return {
      roleId: null,
      allResource: null, // 所有资源
      allResourceCate: null, // 角色已分配的资源
  ...
  methods: {
    ...
    getCateList() {
      axios
        ...
        .catch(err => {
          this.$message.error("资源分类信息数据获取失败! ! !");
        });
    },
    ...
    getResourceList() {
      axios
        ...
        .then(res => {
          this.allResource = res.data.content.list; // res.data.content是分页数据，res.data.content.list是资源列表
      ...
      axios
        ...
        .then(res => {
          ...
          this.allResource.forEach(item => {
            item.selected = this.getResourceChecked(item.id, allocResource); // 判断已分配的资源
          });
        ...
        .catch(err => {
          console.log(err);
          this.$message.error("获取当前角色拥有的资源列表数据获取失败! ! !");
        });
    },
