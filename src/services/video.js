...
export const getImgAuth = async () => {
  return await request(
    `${bossBase}/course/upload/aliyunImagUploadAddressAdnAuth.json`
  );
};

export const getVideoAuth = async ({ fileName, imageUrl, videoId }) => {
  if (videoId) {
    return await request(
      `${bossBase}/course/upload/refreshAliyunVideoUploadAddressAdnAuth.json`,
      {
        params: {
          fileName,
          imageUrl,
    ...
    return await request(
      `${bossBase}/course/upload/aliyunVideoUploadAddressAdnAuth.json`,
      {
        params: {
          fileName,
          imageUrl
...
export const transCodeResult = async lessonId => {
  return await request(
    `${bossBase}/course/upload/aliyunTransCodePercent.json`,
    {
      params: {
        lessonId
      }
